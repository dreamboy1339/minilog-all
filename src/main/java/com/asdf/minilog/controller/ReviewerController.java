...
/**
 * 검토자(Reviewer) 관리 REST 컨트롤러.
 *
 * <p>{@code /api/v2/reviewers} 경로에서 검토자 권한을 가진 사용자의 조회, 생성, 권한 부여/회수 등을 제공한다. 조회를 제외한 변경 작업은 ADMIN
 * 권한이 필요하다.
 */
@RestController
...
public class ReviewerController {
  ...
  /**
   * 검토자 권한을 가진 사용자 목록을 조회한다. (GET /api/v2/reviewers)
   *
   * @return 검토자 사용자 목록
   */
  @GetMapping
  ...
  /**
   * 검토자 권한을 가진 새 사용자를 생성한다. (POST /api/v2/reviewers, ADMIN 전용)
   *
  ...
  /**
   * 기존 사용자에게 검토자 권한을 추가한다. (POST /api/v2/reviewers/{userId}, ADMIN 전용)
   *
  ...
  /**
   * 검토자 사용자의 정보를 수정한다. (PUT /api/v2/reviewers/{userId}, ADMIN 전용)
   *
  ...
  /**
   * 사용자에게서 검토자 권한을 회수한다. (DELETE /api/v2/reviewers/{userId}, ADMIN 전용)
   *
